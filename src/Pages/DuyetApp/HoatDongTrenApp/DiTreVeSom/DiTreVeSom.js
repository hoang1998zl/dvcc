...
export default function DiTreVeSom() {
  moment.locale("vi");
  let focusRef = useRef("");
  ...
  useEffect(() => {
    dvccService.getSomTre(token).then((res) => {
  ...
  let handleDuyet = (id,type) => {
    ...
    if(type === "diTre"){
      ...
      dvccService.duyetVeSom(token,data).then((res) => {
        setReload(Date.now());
        toast.success("Duyệt về sớm thành công!", {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 2000
        });
      })
  ...
  let handleTuChoi = (id,type) => {
    ...
    if(type === "diTre"){
      ...
      dvccService.tuChoiVeSom(token,data).then((res) => {
        setReload(Date.now());
        toast.success("Từ chối về sớm thành công!", {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 2000
        });
      })
  ...
  let renderStatus = (status,id,nguoiduyet,type,nhanvien) => {
    switch (status){
      case 1: return <div className='flex flex-wrap justify-center items-center gap-2 lg:gap-4'>
            <Popconfirm title="Xác Nhận Từ Chối?" okText="Từ Chối" cancelText=" Huỷ" onConfirm={() => handleTuChoi(id,type)}>
              <button
                type="button"
            ...
            <Popconfirm title="Xác Nhận Duyệt?" okText="Duyệt" cancelText=" Huỷ" onConfirm={() => handleDuyet(id,type)}>
              <button
                type="button"
  ...
  let renderSomTre = () => {
    return somTreList?.slice(current*4,current*4 + 4)?.map((somTre,index) => {
      return <tr className='addRow'>
          ...
          <td className='relative'>
            <div className='flex gap-2 items-center py-2'>
              <img
                ...
                src={somTre?.type === "diTre" ? somTre?.ns_nhanvien_dvcc_xin_di_tre_nv_idTons_nhanvien?.nv_avatar: somTre?.ns_nhanvien_dvcc_xin_ve_som_nv_idTons_nhanvien?.nv_avatar}
                alt=""
              />
              <p className='w-max flex-1 text-left'>
                <strong>{somTre?.type === "diTre" ? somTre?.ns_nhanvien_dvcc_xin_di_tre_nv_idTons_nhanvien?.nv_name: somTre?.ns_nhanvien_dvcc_xin_ve_som_nv_idTons_nhanvien?.nv_name} </strong>
                <span>đã xin {somTre?.type === "diTre" ? "Đi trễ" : "Về sớm"}</span>
              </p>
          ...
          <td className='relative'>
            <p>
              {somTre?.type === "diTre" ? somTre?.gio_di_tre :somTre?.gio_ve_som}
            </p>
            <span
          ...
          <td className='relative min-w-[230px]'>
            <p>{somTre?.type === "diTre" ? somTre?.ns_nhanvien_dvcc_xin_di_tre_nguoi_duyetTons_nhanvien?.nv_name : somTre?.ns_nhanvien_dvcc_xin_ve_som_nguoi_duyetTons_nhanvien?.nv_name}</p>
          ...
          <td>
            {renderStatus(somTre?.status,somTre?.id,somTre?.nguoi_duyet,somTre?.type,somTre?.nv_id)}
          </td>
        </tr>
    })
  }
